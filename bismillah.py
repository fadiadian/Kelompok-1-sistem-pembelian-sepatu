...
class GudangSepatu:
    ...
    def _baca_data_dari_excel(self):
        try:
            ...
            return pd.DataFrame(columns=['jenis', 'merek', 'series', 'ukuran', 'kode', 'jumlah', 'harga']).set_index(['jenis', 'merek', 'series', 'ukuran', 'kode'])
    ...
    def _tulis_data_ke_excel(self):
        with pd.ExcelWriter(self.file_path, mode="a", engine='openpyxl', if_sheet_exists='replace') as writer:
            self.data_sepatu.to_excel(writer, sheet_name='Persediaan1', index=False)

    def tambah_sepatu(self, jenis, merek, series, ukuran, jumlah, harga):
        # Memeriksa apakah sepatu sudah ada dalam stok berdasarkan kombinasi jenis, merek, series, ukuran
        existing_row = self.data_sepatu[
            (self.data_sepatu['jenis'] == jenis) &
            (self.data_sepatu['merek'] == merek) &
            (self.data_sepatu['series'] == series) &
            (self.data_sepatu['ukuran'] == ukuran)
        ]

        if not existing_row.empty:
            # Jika sudah ada, tambahkan jumlah stok
            idx = existing_row.index[0]
            self.data_sepatu.at[idx, 'jumlah'] += jumlah
            self.data_sepatu.at[idx, 'harga'] = harga
        else:
            ...
            self.data_sepatu = pd.concat([self.data_sepatu, new_row], ignore_index=True)
        
        self.data_sepatu = self.data_sepatu.sort_values(by=['jenis', 'merek', 'series', 'ukuran'])
        self._tulis_data_ke_excel()
        print(f"{jumlah} sepatu {series} ukuran {ukuran} berhasil ditambahkan dengan harga {harga}.")

    def kurangi_sepatu(self, jenis, merek, series, ukuran, kode, jumlah):
        if int(self.data_sepatu.loc[self.data_sepatu.kode == kode, 'jumlah']) >= jumlah:
            jumlah_kurang = int(self.data_sepatu.loc[self.data_sepatu.kode == kode, 'jumlah'])
            self.data_sepatu.loc[self.data_sepatu.kode == kode, 'jumlah'] = jumlah_kurang - jumlah
            self._tulis_data_ke_excel()
    ...
    def tampilkan_menu_dari_data(self, jenis_menu):
        df_filtered = self.data_sepatu.loc[self.data_sepatu.jenis == jenis_menu]
        menu_list = []
        count = 1
        ...
        if lihat_menu == 1:
            ...
            if menu_list:
                ...
                if 1 <= choice <= len(menu_list):
                    ...
                    gudang.tambah_sepatu(chosen_item[0], chosen_item[1], chosen_item[2], chosen_item[3], jumlah, chosen_item[5])
                else:
...
def kurangi_stok_setelah_pembayaran(gudang):
    for item in pesanan:
        ...
        gudang.kurangi_sepatu(jenis, merek, series, ukuran,kode, jumlah)
